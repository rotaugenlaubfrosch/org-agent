...
The `email` field is validated after extraction. Validation trims whitespace, then discards blank values and values that do not match a conservative email regex. If crawled website pages exist, the trimmed valid email must also appear in the crawled text case-insensitively. Removed invalid or unconfirmed email values are shown as error-style CLI progress messages.

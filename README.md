...
- Extracts structured fields from crawled website pages with an LLM.
- Returns separate website and registry profiles.
- Reports website crawl `status` as `SUCCESS` or `FAILED` in CLI tables, JSON output, and evaluation tables.

Evidence and reasoning output was intentionally removed. Earlier versions asked the LLM to return supporting evidence entries and short reasoning text for extracted values, but that output was not used in normal workflows. Removing it keeps prompts shorter, reduces structured-output complexity, avoids noisy CLI/JSON output, and makes the code easier to maintain. Crawl logs, progress messages, and the `status` field remain available for operational debugging and for checking whether the website scan completed successfully.
...
The `address_country` field is drafted by the company facts prompt using the instruction: "For address_country, return the country of the organization's headquarters." During validation, explicit country signals in the extracted `address`, such as `Liechtenstein`, `CH-8049`, or `LI-9494`, override that draft value.

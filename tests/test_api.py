...
def test_lookup_normalizes_bare_domain(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def fake_run_lookup(lookup_input, settings, country=None, progress=None):
...
def test_lookup_passes_normalized_country(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ORG_AGENT_LLM_PROVIDER", "ollama")
    monkeypatch.setenv("ORG_AGENT_LLM_MODEL", "llama3.1")
    monkeypatch.setenv("ORG_AGENT_OLLAMA_BASE_URL", "http://localhost:11434")
    monkeypatch.setenv("ORG_AGENT_REQUEST_TIMEOUT", "20")
    ...
    api.lookup_organization("Example Ltd", website="example.com", country=" CH ")

    assert captured["country"] == "ch"


def test_lookup_dispatches_to_browser_use(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ORG_AGENT_LLM_PROVIDER", "ollama")
    monkeypatch.setenv("ORG_AGENT_LLM_MODEL", "llama3.1")
    monkeypatch.setenv("ORG_AGENT_OLLAMA_BASE_URL", "http://localhost:11434")
    monkeypatch.setenv("ORG_AGENT_REQUEST_TIMEOUT", "20")

    from org_agent import browser_use_lookup

    captured = {}

    async def fake_run_browser_use_lookup(lookup_input, settings, country=None, progress=None):
        captured["website"] = str(lookup_input.website)
        captured["country"] = country
        return api.LookupResult(
            website_profile=api.OrganizationProfile(
    ...
    monkeypatch.setattr(browser_use_lookup, "run_browser_use_lookup", fake_run_browser_use_lookup)

    api.lookup_organization("Example Ltd", website="example.com", country="CH", browser_use=True)
